...
void loop() {
  ...
  */
  ...
  while(routeCounter < routeNumber){
    ...
    // Position the robot at the junction just before the pick up point
    
    Serial.print("At the pick up junction, turning");
    Serial.println(final_turns[routeCounter]);
    approach_block(final_turns[routeCounter]);
    // Turn into the pick up point
